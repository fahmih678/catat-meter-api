...
function setupModalAccessibility() {
    ...
    modals.forEach(modalId => {
        ...
        modal.addEventListener('show.bs.modal', function() {
            // Set aria-modal before the modal is shown
            modal.setAttribute('aria-modal', 'true');
        });

        modal.addEventListener('shown.bs.modal', function() {
            // Ensure proper focus management after modal is fully shown
            const focusableElements = modal.querySelectorAll(
                'input:not([readonly]):not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), [tabindex]:not([tabindex="-1"])'
            );
            if (focusableElements.length > 0) {
                // Find the first focusable element that's not the close button
                const firstInput = Array.from(focusableElements).find(el =>
                    !el.classList.contains('btn-close') && el.type !== 'button'
                ) || focusableElements[0];
        ...
        modal.addEventListener('hide.bs.modal', function() {
            // Remove aria-modal when hiding
            modal.removeAttribute('aria-modal');
            // Don't manipulate aria-hidden - Bootstrap handles this correctly
        });
    });
}
