...
class RegisteredMonthController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Validasi tahun, bulan, pam yang sama
            $periodMonth = date('Y-m', strtotime($request->period));
            $exists = RegisteredMonth::where('pam_id', $request->pam_id)
                ->whereRaw('DATE_FORMAT(period, "%Y-%m") = ?', [$periodMonth])
                ->exists();
            if ($exists) {
                return $this->errorResponse('Failed to create month The period has already been taken.', 400);
            }
            // Get total customers
            $totalCustomers = Customer::where(['pam_id' => $request->pam_id, 'is_active' => true])->count();
            // Set default values
            $validateData += [
                'pam_id' => $request->pam_id,
