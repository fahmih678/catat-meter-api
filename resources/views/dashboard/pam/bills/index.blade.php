                                            ...
                                            <div class="btn-group-sm">
                                                @if ($bill->status === 'pending')
                                                    <button type="button" class="btn btn-outline-success"
                                                        onclick="markAsPaid({{ $bill->id }})" title="Tandai Lunas">
                                                ...
                                                @if ($bill->status === 'paid')
                                                    <button type="button" class="btn btn-outline-warning"
                                                        onclick="cancelPayment({{ $bill->id }}, '{{ $bill->bill_number }}', '{{ $bill->customer->name ?? 'Pelanggan' }}')"
...
<script>
    function markAsPaid(billId) {
        ...
        document.getElementById('markAsPaidBillId').value = billId;
        modal.show();
    }

    function confirmMarkAsPaid() {
        const billId = document.getElementById('markAsPaidBillId').value;
        ...
        // Show loading state
        const button = document.getElementById('confirmMarkAsPaidBtn');
        const originalHtml = button.innerHTML;
        button.innerHTML = '<i class="bi bi-arrow-clockwise spin"></i> Memproses...';
        button.disabled = true;

        // Send request to mark as paid
        fetch(`{{ url('/pam/' . $pam->id . '/bills') }}/${billId}/mark-as-paid`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Show error message
                    alert('Gagal menandai lunas: ' + (data.message || 'Terjadi kesalahan'));
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert('Gagal menandai lunas. Terjadi kesalahan jaringan.');
            })
            .finally(() => {
    ...
    function cancelPayment(billId, billNumber, customerName) {
        // Set modal data
        document.getElementById('cancelPaymentBillId').value = billId;
    ...
    function confirmCancelPayment() {
        const billId = document.getElementById('cancelPaymentBillId').value;

        // Show loading state
        const button = document.getElementById('confirmCancelPaymentBtn');
        const originalHtml = button.innerHTML;
        button.innerHTML = '<i class="bi bi-arrow-clockwise spin"></i> Membatalkan...';
        button.disabled = true;

        // Send request to cancel payment
        fetch(`{{ url('/pam/' . $pam->id . '/bills') }}/${billId}/cancel-payment`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content')
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Show success modal
                    showSuccessModal('Pembayaran Dibatalkan!', data.message);
                } else {
                    // Show error modal
                    showErrorModal('Gagal Membatalkan Pembayaran', data.message || 'Terjadi kesalahan');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showErrorModal('Gagal Membatalkan Pembayaran', 'Terjadi kesalahan jaringan.');
            })
            .finally(() => {
                // Restore button state
                button.innerHTML = originalHtml;
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="markAsPaidModalLabel">
                <i class="bi bi-check-circle me-2"></i>Tandai Tagihan Lunas
            </h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <p class="mb-3">Silakan pilih metode pembayaran untuk menandai tagihan ini sebagai lunas:</p>

            <input type="hidden" id="markAsPaidBillId" value="">

            <div class="mb-3">
                <label for="paymentMethodSelect" class="form-label">Metode Pembayaran <span
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-success" id="confirmMarkAsPaidBtn"
                ...
                <i class="bi bi-check me-1"></i>Konfirmasi Lunas
            </button>
        </div>
    </div>
</div>
    ...
    <div class="modal-content">
        <div class="modal-header bg-warning text-dark">
            <h5 class="modal-title" id="cancelPaymentModalLabel">
                <i class="bi bi-exclamation-triangle me-2"></i>Konfirmasi Pembatalan Pembayaran
            </h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <div class="alert alert-warning d-flex align-items-center" role="alert">
                ...
                <div>Anda akan membatalkan pembayaran untuk tagihan ini.</div>
            </div>

            <div class="mb-3">
                <strong>Informasi Tagihan:</strong>
                <ul class="mb-0 mt-2">
                    <li><strong>No. Tagihan:</strong> <span id="cancelBillNumber" class="text-primary"></span>
                    </li>
                    <li><strong>Pelanggan:</strong> <span id="cancelCustomerName" class="text-primary"></span>
                    </li>
                </ul>
            </div>
            ...
            <div class="mb-3">
                ...
                <ul class="mb-0 mt-2">
                    <li><i class="bi bi-arrow-repeat text-warning me-1"></i> Mengubah status tagihan menjadi
                        "Menunggu"</li>
                    <li><i class="bi bi-arrow-repeat text-warning me-1"></i> Mengubah status pembacaan meter
                        menjadi "Menunggu"</li>
                    <li><i class="bi bi-x-circle text-danger me-1"></i> Menghapus informasi pembayaran</li>
                </ul>
            </div>

            <div class="alert alert-info d-flex align-items-center" role="alert">
                <i class="bi bi-info-circle me-2"></i>
                <div>Tagihan dapat ditandai lunas kembali jika diperlukan.</div>
            </div>

            <input type="hidden" id="cancelPaymentBillId" value="">
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                <i class="bi bi-x-circle me-1"></i>Tutup
            </button>
            <button type="button" class="btn btn-warning" id="confirmCancelPaymentBtn"
                onclick="confirmCancelPayment()">
                <i class="bi bi-arrow-counterclockwise me-1"></i>Ya, Batalkan Pembayaran
            </button>
        </div>
    </div>
</div>
